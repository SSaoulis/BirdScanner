...
The actual mp4 encode depends on the OpenCV FFmpeg backend, which is not
guaranteed in every CI environment, so the handoff-to-encode is exercised by
monkeypatching ``_encode``. One best-effort test drives the real encoder but
only asserts on the file when the backend produced one.
"""
...
def _frame(value: int) -> np.ndarray:
    """A tiny solid-colour frame tagged by ``value``."""
    return np.full((4, 4, 3), value, dtype=np.uint8)


def test_pre_roll_buffer_is_bounded():
...
def test_encode_runs_after_post_roll(monkeypatch, tmp_path):
    ...
    rec.add_frame(_frame(0))  # seeds the pre-roll
    dest = str(tmp_path / "clip.mp4")
    assert rec.trigger(dest) is True

    rec.add_frame(_frame(1))  # crosses the post-roll boundary → handoff
...
def test_add_frame_without_trigger_does_not_encode(monkeypatch):
...
def test_real_encode_does_not_raise(tmp_path):
    ...
    frames = [_frame(i * 40) for i in range(3)]
