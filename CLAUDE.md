...
- `frontend/src/api.ts` — typed fetch wrappers for all `/api/*` endpoints (including `detections.delete(id)` → `DELETE /api/detections/{id}`); exports `Detection`, `SystemStatus`, `SpeciesSummary` interfaces plus `timeAgo` and `formatUptime` helpers
...
- `frontend/src/components/Lightbox.tsx` — full-screen modal showing the full-res image on open; Esc/arrow-key keyboard navigation; prev/next arrows; caption bar with species, confidence, time-ago, a download link, and a **Delete** action (confirm → `api.detections.delete` → `onDelete(id)`); `Timeline`/`Gallery` pass `onDelete` through from `History`
...
**`src/camera_server.py`** — detector control HTTP server (camera snapshots + detection deletion):
...
- `start_camera_server(picam2, port, delete_detection=None)` — runs a stdlib `ThreadingHTTPServer` on a background daemon thread serving `GET /capture` (returns a fresh JPEG) and `DELETE /detections/{id}` (deletes a detection via the optional `delete_detection` callback, returning 204/404/500); `camera_server_port()` reads the port from `CAMERA_SERVER_PORT` (default 8000)
- Wired into `main.py` after `picam2.start(...)` so the read-only API can surface a live "Test Camera" image **and** delete detections even though the detector owns the camera + read-write data volume exclusively; the delete callback is bound to the same engine the `DetectionWriter` uses (see `db/deleter.py`). Shut down on `KeyboardInterrupt`. NB: because the server starts only after the camera initialises, deletes (like snapshots) are unavailable while the camera is down
