...
- `hooks/useMediaQuery.ts` — `useMediaQuery(query)` + `useIsDesktop()` (`lg` = 1024px), used by `Lightbox` to choose side-by-side vs stacked panels and to gate the mobile swipe filmstrip.
...
- Components: `DetectionCard` (thumbnail + both confidences + corrected mark; responsive full-width row on mobile / vertical plate on `sm`+), `Lightbox` (full-screen overlay: still + specimen-label caption, box-overlay toggle, Photo/Video toggle, Correct-ID pencil → `SpeciesPicker`, Field-guide + Advanced-stats panels — side-by-side on desktop, stacked segmented control below `lg`; prev/next via edge arrows + ←/→ on desktop; on mobile the floating arrows (`lg`-only) give way to a **physical, finger-tracking swipe** — the card is the live centre slot of a three-up filmstrip `[prev | current | next]` (`components/MobileSwipeStrip.tsx`, lazy-loaded so `motion` never lands in the initial/desktop bundle) that follows the drag, committing to a neighbour past a distance/velocity threshold (via the same `onPrev`/`onNext` funnel as the arrows/keys) or rubber-banding back; neighbour slots are non-interactive `NeighborPreview`s (thumbnail → full-res on drag-start) that mirror the *full* live-card chrome (control bar, specimen-label actions, panel switcher) rendered inert — via the shared presentational components in `components/LightboxChrome.tsx` (`PlateControlBar`/`SpecimenLabel`/`MobilePanelTabs`, also used by the live card so the two never drift) in each control's default state — so the overlay controls are on screen *as* a plate slides in instead of popping in ~1 s after the commit; the neighbour still "wakes up" into the fully interactive card once centred. The **committing** settle is a short bounded tween (`easeOut`, ~0.26 s) so `onComplete` swaps the real card in promptly; the long-tailed spring is kept only for the rubber-band-back cancel. A "N / total" position chip shows on mobile, tucked into the top control bar's right group immediately left of the Close button so it stays clear of the left-hand view controls (optional `position` prop). The filmstrip needs the neighbour records, so the list-owning parents (`Timeline`/`Gallery`/`Dashboard`) also pass `prevDetection`/`nextDetection`. A prev/next swaps the sighting immediately and covers the full-res still/clip fetch with a breathing blur-up loader — a blurred low-res thumbnail that pulses (`animate-plate-breathe`) then resolves into the sharp plate (`animate-plate-develop`, desktop only — on mobile the filmstrip slide is the transition, and the still's cached-image check skips the loader flash on a committed swipe), the media element's own load event (`onLoad`/`onCanPlay`, `onWaiting` re-buffer) flipping `photoReady`/`videoReady`; while loading the plate is pinned to the last-known `imgSize` so the footprint never jumps, and the box overlay is gated on `photoReady` so it fades in with the sharp image), `SpeciesPicker` (keyboard type-ahead over the vocabulary + add-a-new-species), plus `TodaySummary` (day-totals panel — sightings/species/most-common)/`ExpectedThisWeek`/`FeaturedDetection` (hero image + per-sighting stats)/`AdvancedStats`/`SystemMonitor`/`NetworkMonitor`/`CropEditor`/`Timeline`/`Gallery`/`FileDownloader`, and the Nivo charts under `components/charts/` with shared `charts/theme.ts` + `charts/format.ts` (CVD-validated 6-hue categorical palette — 6 is the safe ceiling, which is why the timeline requests top-6 + `Other`).
