...
- `frontend/src/api.ts` — typed fetch wrappers for all `/api/*` endpoints (including `detections.delete(id)` → `DELETE /api/detections/{id}`); exports `Detection`, `SystemStatus`, `SpeciesSummary`, `CropState`, `NormalizedBox` interfaces plus `timeAgo` and `formatUptime` helpers. `api.camera` exposes `snapshotUrl`, `fullSnapshotUrl`, `getCrop`, and `setCrop` (POSTs a normalized box or `{reset:true}`)
