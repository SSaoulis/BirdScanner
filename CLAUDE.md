...
- `build_name_mapping(geomodel_labels, classifier_labels, overrides=None)` — returns `(mapping, unmatched)`: `mapping` is `geomodel_common_name -> classifier_label` (keyed by the geomodel's name since that is what a caller translates *from*) for every auto-matched class **plus** the hand-curated `overrides`; `unmatched` is the sorted classifier labels still without a counterpart (58 as of V3.0.3 — the reliable long-term join key is the **scientific name**, which the classifier's `class_to_idx.json` currently lacks). First geomodel row wins on a normalised-key collision, so the map is stable across re-runs
- The generated `assets/labels/geomodel_classifier_map.json` (a flat, sorted `{geomodel_common_name: classifier_label}` object) is **curated source** — a `!assets/labels/geomodel_classifier_map.json` negation keeps it tracked despite the repo-wide `*.json` ignore. Build/refresh it with `tools/build_geomodel_map.py` (below)
...
**`tools/build_geomodel_map.py`** — offline builder for the geomodel↔classifier label crosswalk (**not** runtime; pure stdlib, no network). Reads the geomodel labels (`assets/labels/BirdNET+_Geomodel_V3.0.3_Global_12K_Labels.txt`) + the classifier's `assets/models/convnext_v2_tiny.onnx_class_to_idx.json`, calls `geomodel.build_name_mapping`, and writes `assets/labels/geomodel_classifier_map.json` (the sorted `{geomodel_common_name: classifier_label}` map). It **prints the unmatched classifier classes as a table** for a human to curate (the ~58 genuine synonyms normalisation can't resolve). Curated entries are added straight into the JSON as more `"<geomodel_common_name>": "<classifier_label>"` pairs and are **preserved** across re-runs — `load_existing_overrides` treats any existing entry whose classifier label isn't a fresh auto-match as a curated override and feeds it back in. `--check` prints the table without writing. Run with the project `.venv` (it imports `birdscanner.ml.geomodel`, which imports `onnxruntime`)
