...
- **`.pylintrc`** (root) lints the source tree. It silences the same Pi-only / untyped
  ...
  **decomposed** (see below). Do not add a `# pylint: disable=too-many-*` to silence one
  ...
  `DetectionRecord` import in `db/database.py`, the `wrong-import-position` in
  `tests/tools/test_build_species_reference.py`, and the `unused-argument` in
  `ml/geolocation.PlaceholderGeolocationModel.predict_week` (the placeholder is
  location/week-independent but must match the real model's signature).
...
### Geolocation prior (third model — groundwork only, update not yet wired)

A **third model** is being added: a geolocation model mapping `(latitude, longitude,
week_of_year) → a probability vector over species` (each species' presence likelihood at
that place/time). It will become the **prior** in a Bayesian update against the
classifier's softmax **likelihood** (`posterior ∝ prior × likelihood`) to suppress
out-of-range false positives. Because the output depends only on location + week, the
**52 weekly vectors are precomputed once at startup** and cached in a **separate SQLite
DB** (`GEO_DB_PATH`, default `/data/geo_priors.db`), keyed by ISO week; a classification
then looks up the current week's vector instead of recomputing per detection.

**What is built now is the groundwork** — the cache DB, the startup generation, the
lat/lon settings + UI, and a stub model. The real geo model file is **not transferred
yet**, and the in-pipeline Bayesian multiply (and exposing the classifier's full
probability vector) is a **deliberate follow-up** — `Classifier.classify()` still returns
only the top-1 species today. The pieces:
- `birdscanner/ml/geolocation.py` holds the model seam (`GeolocationModel` protocol +
  `PlaceholderGeolocationModel`, a uniform-vector no-op prior until the real model lands).
- `birdscanner/ml/geo_priors.py` holds the pure coordination (`generate_all_weeks`,
  `species_signature`, `current_week`).
...
birdscanner/
  detector/   Pi-only camera + hardware control and the entry point (main, camera,
              gating, geo, config, camera_server, crop, crop_controller, settings,
  ...
  ml/         platform-independent inference (object_detection, detection_utils,
              ...
              geolocation, geo_priors)
  api/        FastAPI REST API + routers (was backend/)
  db/         SQLite persistence (models/database/writer/deleter, geo_store)
...
**`birdscanner/ml/geolocation.py`** — the geolocation model seam (pure `ml/`, unit-tested in `tests/ml/test_geo_priors.py`):
- `GeolocationModel` — a `Protocol` for a model that maps `(latitude, longitude, week)` → a `float32` presence-probability vector aligned to a `species_order` (with a `model_version` tag). `predict_week(lat, lon, week)` returns the vector for one ISO week
- `PlaceholderGeolocationModel` — the stub shipped until the real model is transferred: returns a **uniform** vector for every location/week (a Bayesian no-op prior), so the DB/generation/settings/wiring can be built and tested ahead of the model. Swapping in the real model replaces only this class. Its `predict_week` carries the codebase's third `unused-argument` block-disable (the placeholder ignores its inputs but must match the protocol)
- `load_species_order(class_to_idx_path)` — reads the classifier's `{class_name: index}` JSON and returns the species names in **index order**, so the geo vectors stay aligned to the classifier's class indices (the basis for the future element-wise prior × likelihood multiply)

**`birdscanner/ml/geo_priors.py`** — pure (I/O-free) coordination for the geo-prior cache (unit-tested in `tests/ml/test_geo_priors.py`):
- `generate_all_weeks(model, lat, lon, on_progress=None)` — computes the presence vector for every week `1..52`, invoking `on_progress(week)` after each so the caller can log progress
- `species_signature(species_order)` — a stable SHA-256 of the ordered species list, stored with the cache so a changed/reordered class set forces regeneration
- `current_week()` — the current ISO week, clamped so week 53 folds onto 52; `WEEKS_IN_YEAR = 52`
- The eventual `apply_geo_prior(likelihood, prior)` Bayesian multiply will live here — **deferred** until the real model + full-vector classifier output land
...
- `birdscanner/db/geo_store.py` — `GeoPriorStore`: the **separate** geolocation-prior DB (`GEO_DB_PATH`, default `/data/geo_priors.db`). Owns its **own** SQLAlchemy `MetaData` (`_GEO_METADATA`) so it never cross-creates tables with the `detections` SQLModel metadata (the two DBs are distinct files). Two tables: `geo_meta` (one row: `latitude`, `longitude`, `species_order` JSON, `species_signature`, `model_version`, `generated_at`) and `geo_week_prior` (52 rows keyed by ISO `week`, each `vector` a `float32` blob). API: `read_meta()`, `write_priors(meta, vectors)` (replaces the whole cache in one transaction), `get_week_vector(week)`. `geo_db_path()` reads the env var. Detector-only (owns the read-write volume); the API never reads it
...
- `apply_settings_to_config(settings)` runs **once at startup** (before the pipeline is wired) so restart-only settings shape the initial build; it pushes every field onto the `config` singleton (incl. `config.latitude`/`config.longitude`) + `classification_pipeline.IMAGE_DIR`. `apply_settings_to_context(settings, context)` applies the save-side fields (`save_confidence_threshold`, lower-cased `ignore_species`) to the `PipelineContext` — these live on the context (not `config`) because they are consumed inside `ml/`, which must not import `detector/`
...
- `main()` reads as a short startup script (`load_settings`/`apply_settings_to_config` → `configure_logging` → `make_engine`/`init_db` → `wait_for_camera`/`prepare_intrinsics` → `setup_classifier` → `bootstrap_geo_priors` → `build_gating` → `DetectionWriter`/`build_manager`/`apply_settings_to_context`/`SettingsController` → `build_camera` → `_start_control_server` → `_run_capture_loop`). `bootstrap_geo_priors` runs right after the classifier is set up (it needs the class list for the species signature) and precomputes/caches this location's 52 weekly geolocation-prior vectors — a synchronous startup step that is a no-op cache hit unless the location changed (see `detector/geo.py`). The persisted **settings overlay** is loaded and pushed onto `config` **before** anything is wired so restart-only settings shape the build; a `SettingsController` is then handed to `_start_control_server`. `_schedule_restart` (the `/restart` callback) does a delayed `os._exit(0)` from a timer thread so the HTTP response flushes first and Docker's `restart: unless-stopped` policy relaunches the detector to pick up restart-only settings (outside Docker it just exits). The camera bring-up lives in `detector/camera.py` and the classification-pipeline wiring in `detector/gating.py`; `main.py` keeps only `_start_control_server`, `_run_capture_loop`, `_shutdown`, and `main`. A `KeyboardInterrupt` out of `_run_capture_loop` triggers `_shutdown`
...
- The `data` Docker volume is the single source of truth: the `detector` service writes images, the per-detection **mp4 clips** (alongside the stills under `$IMAGE_DIR/{species}/`), the SQLite DB, the crop-region JSON (`CROP_CONFIG_PATH`, default `/data/crop.json`), the **runtime settings overlay** (`SETTINGS_PATH`, default `/data/settings.json`), and the **geolocation-prior cache** (`GEO_DB_PATH`, default `/data/geo_priors.db` — a separate SQLite DB of the 52 precomputed weekly presence vectors, regenerated at startup only when the configured latitude/longitude changes); the `api` service mounts it read-only. Both services share `env_file: .env`, so `GEO_DB_PATH` is picked up automatically — but only the `detector` reads/writes it.
